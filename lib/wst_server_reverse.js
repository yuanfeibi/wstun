...
wst_server_reverse = function(https_flag, key, cert) {

  if(https_flag == "true"){
    
    //HTTPS
    console.log("[SYSTEM] - " + (new Date()) + " WSTT over HTTPS");
    var https = require('https');
    var fs = require('fs');

    require("../lib/https_override");
    
    flag_https = https_flag;

    try{
      // certificates loading from file
      this.s4t_key = fs.readFileSync(key, 'utf8');
      this.s4t_cert = fs.readFileSync(cert, 'utf8');
    ...
    //HTTP
    console.log("[SYSTEM] - " + (new Date()) + " WSTT over HTTP");
...
wst_server_reverse.prototype.start = function(port) {
  ...
  if (flag_https == "true")
    console.log("[SYSTEM] - " + (new Date()) + " WSTT server starting on: wss://localhost:" + port + " - CERT: \n" + this.s4t_cert);
  else
    console.log("[SYSTEM] - " + (new Date()) + " WSTT server starting on: ws://localhost:" + port);

  //Activate HTTP/S server
  this.httpServer.listen(port, function() {
    console.log("[SYSTEM] - " + (new Date()) + " WSTT server is listening...");
  });
