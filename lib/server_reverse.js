...
wst_server_reverse = function(options) {
  ...
  if(https_flag == "true"){
    ...
    this.httpServer = https.createServer(credentials, this.httpRequestHandler.bind(this));
    ...
    return function(request){
      ...
      if (uri.query.dst != undefined){
        ...
        request.tcpServer.listen(portTcp, function(){
          //close previous ws connection if exists
          _this.closeWSConnectionBy(portTcp);
...
wst_server_reverse.prototype.httpRequestHandler = function(request, response) {
  ...
  var result = /^\/close\/(\d+)\??/.exec(request.url);
  if(result) {
    this.closeWSConnectionBy(result[1]);
    response.writeHead(200, {"Content-Type": "application/json"});
    response.write(`Web socket tunnel related to port ${result[1]} is closed successfully.`);
    response.end();
    return;
  }
