...
wst_server_reverse = function(options) {
  ...
  if(https_flag == "true"){
    ...
    this.httpServer = https.createServer(credentials, dgs(this));
    ...
    this.httpServer = http.createServer(dgs(this));
  ...
  this.wsServerForControll.on('request', (function(_this){
    return function(request){
      ...
      if (uri.query.dst != undefined){
        //Secure connection by token
        if(uri.query.token != _this.token) {
          logger.info("[SYSTEM] --> WS connection rejected because of invalid token");
          request.reject();
          return;
        }
        ...
        request.tcpServer.listen(portTcp, function(){
          _this.wsConnMap[portTcp] = request.wsConnectionForControll;
          // gateway client id exists
          if(uri.query.trayId) {
            _this.trayList.push({
              id: uri.query.trayId,
              name: uri.query.trayName,
              portTcp: portTcp
            });
          }
        });
        ...
        request.wsConnectionForControll.on('close', function(reasonCode, description) {
          ...
          _this.unregisterClientBy(portTcp);
        });
...
wst_server_reverse.prototype.closeWSConnectionBy = function(portTcp) {
  var wsConnection = this.wsConnMap[portTcp];
  if (wsConnection && typeof wsConnection.close === 'function') {
    wsConnection.close();
    this.wsConnMap[portTcp] = null;
    this.unregisterClientBy(portTcp);
  }
};

wst_server_reverse.prototype.unregisterClientBy = function(portTcp) {
  var ret = null;
  for (var i = 0; i < this.trayList.length; i++) {
    if(this.trayList[i].portTcp == portTcp) {
      ret = this.trayList.splice(i,1)[0];
      break;
    }
  }
  return ret;
