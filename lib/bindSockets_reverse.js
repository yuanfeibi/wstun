...
bindSockets = function(wsconn, tcpconn) {
  ...
  tcpconn.on("data", function(buffer) {
    //DEBUG
    console.log('[SYSTEM] - ' + (new Date()) + ' --> TCP DATA:\n'+buffer); //console.log(JSON.stringify(buffer));
    return wsconn.sendBytes(buffer);
  });
  
  wsconn.on("error", function(err) {
    return console.log('[SYSTEM] - ' + (new Date()) + ' --> WebSocket Error: ' + err);
  });
  ...
  wsconn.on('close', function(reasonCode, description) {
    console.log('[SYSTEM] - ' + (new Date()) + ' --> WebSocket Peer ' + wsconn.remoteAddress + ' disconnected for: \"'+description+'\"');
    return tcpconn.destroy();
  });
